 ...
 * one Matrix API request with some origins and some destinations costs `origins * destinations / 2` credits if `origins` or `destinations` are less than 20. For bigger matrices we use the cheaper formular `MAX_OF(origins, destinations) * 10`. For example you have 2 origins and 10 destinations the charged credits are `2 * 10 / 2 = 10`. If you have 30 origins and 40 destinations it is `40 * 10 = 400`. If you have one-to-N matrices like 1-to-100 then always the `origins*destinations/2` formula applies.
