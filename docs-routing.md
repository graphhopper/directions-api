...
optimize    | false   | If false the order will be as the order of the point parameters are. If you have more than 2 points you can set this optimize parameter to `true` and the via points will be sorted regarding the minimum overall time. Keep in mind that the location limits of the [Route Optimization API](docs-route-optimization.md) applies and the credit costs are 10 times higher!
