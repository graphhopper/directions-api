...
type | string | - | pickup | Specifies whether a service is a `pickup` or a `delivery`. This makes a difference if items are load or unloaded. If it is a `pickup`, it is load and will stay in the vehicle for the rest of the route. If it is a `delivery`, items are implicitly loaded at the beginning of the route and will stay in the route until `delivery`.
