...
required_skills | array | - | no_skills | Specifies an array of required skills, i.e. array of string (not case sensitive). For example, if this service needs to be conducted by a technician that have a `drilling_machine` and a `screw_driver` then specify the array as follows: `["drilling_machine","screw_driver"]`. This means that the service can only be done by vehicle (technician) that has `drilling_machine` AND a `screw_driver` in its skill array. Otherwise it remains unassigned.
